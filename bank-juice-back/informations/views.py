...
def user_recommend_deposit(request):
    if request.method == 'GET':
        ...
        # 클러스터링 데이터 준비 및 실행
        data, user_ids = prepare_cluster_data()
        kmeans = perform_clustering(data, k=3)  # 3개의 클러스터로 나눔
        
        # 로그인 사용자의 클러스터 식별
        user_index = user_ids.index(login_user.id)
        user_cluster = kmeans.labels_[user_index]
        
        # 동일 클러스터의 사용자 검색
        same_cluster_user_ids = [
            user_ids[i] for i, label in enumerate(kmeans.labels_) if label == user_cluster
        ]

        # 동일 클러스터 사용자의 추천 상품 수집
        recommend_products = set()
        for user_id in same_cluster_user_ids:
...
def prepare_cluster_data() :
    ...
    for user in users:
        data.append([user.age, user.salary])  # 클러스터링에 사용할 속성
        user_ids.append(user.id)  # 사용자 ID 저장 (클러스터링 결과 매핑에 필요)
